...
app.get('/AutoComplete/:location', (req, response)=>{      
	...
	request(domain, (err, res, body)=>{
		if(!err){
            let result= JSONproccessauto(body);
...
app.get('/Details/:itemId' , (req,resp)=>{
    ...
    request.get(apicall2, function(err,res, body){
        if(!err){
            let de = JSON.parse(body);
            var result = ebayDetailJSON(body);
...
function JSONproccessauto(body){
    ...
    if(auto.postalCodes.length == 0 || auto.postalCodes === undefined ){
        ...
        for(i = 0; i < auto.postalCodes.length; i++){      
            if(auto.postalCodes.length == 0  || auto.postalCodes === undefined || auto.length == 0){
                reccomend = [];        
            }
            else {
...
function ebayJSON(body){
	...
	if(jsonfile != null){
		if(jsonfile.findItemsAdvancedResponse[0].paginationOutput[0].totalEntries != 0 ){
		var items = jsonfile.findItemsAdvancedResponse[0].searchResult[0].item;
        //console.log(items.length);
		//console.log(jsonfile.findItemsAdvancedResponse[0].paginationOutput[0].totalEntries);
		
        for (var i = 0; i < items.length; i++) {
            var newJson = {};
            var item  = items[i];
			if(item.hasOwnProperty('galleryURL')){
				newJson.pic = item.galleryURL[0];
			}
			if(!item.hasOwnProperty('galleryURL')){
				newJson.pic = "N/A"
			}
			if(item.hasOwnProperty('title')){
				newJson.title = item.title[0];
			}
			if(!item.hasOwnProperty('title')){
				newJson.title = "N/A"
			}
			...
			if(item.hasOwnProperty('sellingStatus')){
				newJson.price = "$" + item.sellingStatus[0].currentPrice[0].__value__;
			}
			if(!item.hasOwnProperty('sellingStatus')){
				newJson.price = "N/A"
			}
			...
			if(item.hasOwnProperty('shippingInfo')){
				var shippingOption = item.shippingInfo[0].shippingServiceCost[0].__value__;
				if(shippingOption == 0.0){
                    newJson.shippingOption = "Free Shipping";
                }
			...
			//seller
			if(item.hasOwnProperty('sellerInfo')){
				newJson.seller = item.sellerInfo[0].sellerUserName[0];
			}
			if(!item.hasOwnProperty('sellerInfo')){
				newJson.seller = "N/A"
			}
			...
			if(item.hasOwnProperty('viewItemURL')){
				newJson.viewitem = item.viewItemURL[0];
			}
			if(!item.hasOwnProperty('viewItemURL')){
